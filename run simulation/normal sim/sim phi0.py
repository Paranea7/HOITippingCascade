...
def parse_args():
    ...
    p.add_argument('--s', type=int, default=50, help='system size (number of species)')
    # 新增：mu_e 与 sigma_e 列表（兼容单值）
    p.add_argument('--mu-e-list', type=str, default='0.5,1.0', help='comma-separated mu_e values')
    p.add_argument('--sigma-e-list', type=str, default='0.5', help='comma-separated sigma_e values')
    p.add_argument('--mu-d', type=float, default=0.1, help='(single) mu_d value - if you want multiple mu_d, pass comma list and edit code accordingly')
    ...
    p.add_argument('--t-steps', type=int, default=2000, help='RK4 steps')
    p.add_argument('--sims-per-point', type=int, default=10, help='simulations per phi point')
    p.add_argument('--n-workers', type=int, default=max(1, cpu_count()), help='parallel workers')
